  ...
  <v-container text-center>
    <v-layout row wrap>
      ...
      <v-flex xs12 sm6 md8 lg8>
        <v-card>
          <v-card-text>
            <v-form>
              ...
              <v-dialog ref="dateDialog" :return-value.sync="record.date" v-model="showDateDialog" persistent
                ...
                <template v-slot:activator="{ on }">
                  <v-text-field name="date" label="Vencimento" prepend-icon="event" type="text" readonly
                    :value="formattedDate" v-on="on"></v-text-field>
                </template>
              ...
              <v-select name="category" label="Categoria" prepend-icon="class" :items="categories" item-text="description"
                item-value="id" v-model="$v.record.categoryId.$model">
              ...
              <v-text-field v-show="showTagsInput" name="tags" label="Tags (separadas por vírgula)" prepend-icon="label"
            ...
            <v-tooltip left>
              <template v-slot:activator="{ on }">
                <v-btn icon small class="mr-3" v-on="on">
                  <v-icon :color="color" @click="showTagsInput = !showTagsInput">label</v-icon>
                </v-btn>
              </template>
            ...
            <v-tooltip right>
              <template v-slot:activator="{ on }">
                <v-btn icon small>
                  <v-icon :color="color" @click="showNoteInput = !showNoteInput" v-on="on">note</v-icon>
                </v-btn>
              </template>
        ...
        <v-btn color="secondary" large fab class="mt-4 mr-2" @click="$router.back()">
          <v-icon>close</v-icon>
        </v-btn>
